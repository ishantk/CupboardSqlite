...
public class MainActivity extends AppCompatActivity
    ...
    private void insertRecords() {
        ...
        Book book1 = new Book("Advance Android", "Satya", 3000);
        Book book2 = new Book("Advance iOS", "Bill", 3000);
        Book book3 = new Book("Advance Java", "John", 3000);

        long rowId1 = cupboard().withDatabase(sqLiteDatabase).put(book1);

        if (rowId1 != -1) {
            Toast.makeText(this, "Insert Success " + rowId1, Toast.LENGTH_SHORT).show();
        }

        long rowId2 = cupboard().withDatabase(sqLiteDatabase).put(book2);
        if (rowId2 != -1) {
            Toast.makeText(this, "Insert Success " + rowId2, Toast.LENGTH_SHORT).show();
        }

        long rowId3 = cupboard().withDatabase(sqLiteDatabase).put(book3);
        if (rowId3 != -1) {
            Toast.makeText(this, "Insert Success " + rowId3, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void readRecords() {
        ...
        if (books != null && books.size() > 0) {
            for (Book book : books) {
                Toast.makeText(this, book.name + " " + book.author + " " + book.price, Toast.LENGTH_SHORT).show();
    ...
    private void updateRecords() {
        ...
        int numRowsUpdated = cupboard().withDatabase(sqLiteDatabase).update(Book.class, values, "name = ?", "Advance Java");
    ...
    private void deleteRecords() {
        int numRowsDeleted = cupboard().withDatabase(sqLiteDatabase).delete(Book.class, "name = ?", "Advance Java");
